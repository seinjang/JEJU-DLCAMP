...
def create_input_ops(dataset,
    ...
    with tf.device("/cpu:0"), tf.name_scope(scope):
        ...
        img, q, p, a = dataset.get_data(data_id[0])

        def load_fn(id):
            # image [n, n], q: [m], a: [l]
            img, q, p, a = dataset.get_data(id)
            return (id, img.astype(np.float32), q.astype(np.float32), p.astype(np.float32), a.astype(np.float32))

        input_ops['id'], input_ops['img'], input_ops['q'], input_ops['p'], input_ops['a'] = tf.py_func(
        ...
        input_ops['p'].set_shape(list(p.shape))
