...
class Model(object):
    ...
    def __init__(self, config,
        ...
        self.p_1_dim = self.config.data_info[4]
        self.p_2_dim = self.config.data_info[5]
        self.a_dim = self.config.data_info[6]
        ...
        self.p = tf.placeholder(
            name='p', dtype=tf.float32, shape=[self.batch_size, self.p_1_dim, self.p_2_dim],
        )
    ...
    def build(self, is_train=True):
        ...
        def g_theta(o_i, o_j, q, p, scope='g_theta', reuse=True):
            with tf.variable_scope(scope, reuse=reuse) as scope:
                if not reuse: log.warn(scope.name)
                g_1 = fc(tf.concat([o_i, o_j, q, p], axis=1), 256, name='g_1')
        ...
        def CONV(img, q, p, scope='CONV'):
            with tf.variable_scope(scope) as scope:
                ...
                all_g = []
                for candi in range(p.get_shape().as_list()[1]):
                    each_g = []
                    for i in range(d*d):
                        ...
                        for j in range(d*d):
                            ...
                            if i == 0 and j == 0 and candi == 0:
                                g_i_j = g_theta(o_i, o_j, q, p[:, candi, :], reuse=False)
                            else:
                                g_i_j = g_theta(o_i, o_j, q, p[:, candi, :], reuse=True)
                            each_g.append(g_i_j)

                    each_g = tf.stack(each_g, axis=0)
                    each_g = tf.reduce_mean(each_g, axis=0, name='all_g')
                    if candi == 0:
                        all_g = f_phi(each_g, reuse=False)
                        idx = tf.tile(tf.expand_dims(tf.one_hot(candi,10), axis=0), [self.batch_size, 1])
                        all_g = tf.concat([ tf.to_float(idx), all_g ], axis=1)
                    else:
                        each_g = f_phi(each_g, reuse=True)
                        idx = tf.tile(tf.expand_dims(tf.one_hot(candi,10), axis=0), [self.batch_size, 1])
                        each_g = tf.concat([ tf.to_float(idx), each_g ], axis=1)
                        all_g = tf.concat([all_g, each_g], axis=1)
                return all_g
        ...
        def f_phi(g, scope='f_phi', reuse=True):
            with tf.variable_scope(scope, reuse=reuse) as scope:
                ...
                fc_3 = slim.dropout(fc_2, keep_prob=0.5, is_training=is_train, scope='fc_3/')
        ...
        def final(f, scope='final'):
            with tf.variable_scope(scope) as scope:
                ...
                fi_3 = slim.dropout(fi_2, keep_prob=0.5, is_training=is_train, scope='final_3/')
                fi_4 = fc(fi_3, 10, activation=None, name='final_4')
                return fi_4


        g = CONV(self.img, self.q, self.p, scope='CONV')
        logits = final(g)
